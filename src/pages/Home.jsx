...
const Home = () => {
    ...
    const textBio = "I Am Into FullStack Development"

    return (
        <>
            <main className='flex max-md:flex-col items-center md:justify-center max-md:mt-10 h-screen'>
                {/* TEXT SECTION */}
                <div className="sectionLeft w-full md:w-2/5 allCenter">
                    <GradualSpacing
                        className="font-display text-center text-2xl font-bold tracking-[-0.1em]  text-black dark:text-white lg:text-4xl mb-5"
                        text="Hi  There,"
                    />
                    <GradualSpacing
                        className="font-display text-center text-4xl font-bold tracking-[-0.1em]  text-black dark:text-white lg:text-7xl"
                        text="I   Am   Toufik   Gouri"
                    />
                    <GradualSpacing
                        className="font-display text-center font-bold tracking-[-0.1em]  text-black dark:text-white lg:text-2xl mb-8"
                        text="I Am Into FullStack Development"
                    />
                    {/* Proficiency In Skills*/}
                    <GradualSpacing
                        className="font-display text-center text-xl font-bold tracking-[-0.1em]  text-black dark:text-white lg:text-2xl"
                        text="My Proficiency Is In"
                    />
                    <WordRotate
                        duration={3000}
                        className="lg:text-xl font-bold text-purple-500 uppercase"
                        words={["React.Js", "UI Development", "Creativity", "Interactive Designs", "Landing Page"]}
                    />
                </div>
                ...
                <div className="sectionRight CIRCLE w-full md:w-2/5 allCenter">
                    <div className="relative flex h-[350px] w-full flex-col items-center justify-center overflow-hidden rounded-lg bg-background">
                        ...
                        {Icons.map((val, ind) =>
                            <OrbitingCircles
                                key={ind}
                                className={`size-[35px] md:size-[50px] border-none bg-transparent`}
                                radius={150}
